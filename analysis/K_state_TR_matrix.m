function matrix = K_state_TR_matrix(ch, p, x, R, L)
ch.eta_0 = 2^R-1;

matrix = zeros(3, 3);

matrix(1,1) = Dec00(p)+Dec11(p, ch, L)*x+Dec10(p, ch, L)+Dec22(p, ch, L)*(x^2)+Dec21(p, ch, L)*x+prob_Col_0(p, ch, L);
matrix(1,2) = prob_Col_1(p, ch, L);
matrix(1,3) = prob_Col_2(p, ch, L);

matrix(2,1) = Rec_nop(p, ch, L)*(x^2)+Rec_pot(p, ch, L)*x;
matrix(2,2) = Dec00(p)+Dec10(p, ch, L)+Col_old(p, ch, L);
matrix(2,3) = Col_new(p, ch, L);

matrix(3,1) = Dec11(p, ch, L)*(x^2)+Dec22(p, ch, L)*(x^2)+Dec21(p, ch ,L)*(x^2);
matrix(3,2) = 0.0;
...
term2 = exp(Y);
term3 = exp(Z);
prob = p^2 * ( 1 + 2*term1 - 2*term2 - term3 );
prob = max(prob, 0);
end

function prob = Col_old(p, ch, L)
prob = prob_Col_1(p, ch, L)*0.5+prob_Col_0(p, ch, L);
end

function prob = Col_new(p, ch, L)
prob = prob_Col_1(p, ch, L)*0.5+prob_Col_2(p, ch, L);
end
...
% $P\{E_0^{\mathrm{Col}}\}
function prob = prob_Col_0(p, ch, L)
X = L * log1p( 2 * Col_0(ch) - 1 );
prob = p^2 * exp(X);
end

% $P\{E_1^{\mathrm{Col}}\}
function prob = prob_Col_1(p,ch,L)
A = Col_1(ch) + 2 * Col_0(ch);
B = 2 * Col_0(ch);
X = L * log1p(A - 1);
Y = L * log1p(B - 1);
M = max(X, Y);
...
% $P\{E_2^{\mathrm{Col}}\}
function prob = prob_Col_2(p,ch,L)
A = 2 * (P_2(ch) + Col_1(ch) + Col_0(ch));
B = 2 * Col_0(ch);
C = Col_1(ch) + 2 * Col_0(ch);
X = L * log1p(A - 1);
Y = L * log1p(B - 1);
Z = L * log1p(C - 1);
M = max([X, Y, Z]);
diff_exp = exp(X - M) + exp(Y - M) - 2 * exp(Z - M);
diff_exp = max(diff_exp, 0);
